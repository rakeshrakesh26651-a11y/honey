...
const getFontValue = (value: string | number, isSpacing = false): string => {
  if (typeof value === 'number') {
    if (isSpacing && Math.abs(value) <= 1 && value !== 0) {
      return `${value}em`;
    }
    return `${value}px`;
  }
...
const buildTextCanvas = ({ container, width, height, dpr, props }: BuildTextCanvasArgs): HTMLCanvasElement => {
  ...
  Object.assign(probe.style, {
    ...
    letterSpacing: getFontValue(props.letterSpacing, true),
  ...
  const fontFamily = computed.fontFamily || 'serif, sans-serif';
...
export const WarpText: React.FC<WarpTextProps> = ({
  ...
  const finalStyle: CSSProperties = {
    ...style,
    height: style?.height === '320px' ? 'clamp(180px, 38vw, 320px)' : style?.height
  };
